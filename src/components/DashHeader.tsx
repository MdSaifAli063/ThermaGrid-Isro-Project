...
function NavLink({
  ...
  return (
    <Link
      ...
      search={search}
      onClick={onNavigate}
      className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-4 py-2.5 font-mono text-[11px] font-bold uppercase tracking-wider transition-all ${className} ${
        active
    ...
    <header className="sticky top-0 z-50 border-b border-primary/12 bg-background/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1600px] items-center gap-2 px-4 py-3 sm:gap-3 sm:px-6">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
          <Link
            to="/"
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-border/60 bg-card/40 text-muted-foreground backdrop-blur transition-all hover:border-primary/40 hover:bg-primary/10 hover:text-primary"
          >
            <ArrowLeft className="h-4 w-4" />
          </Link>

          <LogoMark />
          <div className="min-w-0 font-mono text-sm font-bold tracking-tight">
        ...
        {/* Desktop nav */}
        <nav className="hidden flex-1 flex-wrap items-center justify-center gap-1 px-2 xl:flex">
          {ITEMS.map((it) => (
            <NavLink
              key={it.to}
              to={it.to}
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/8 px-3 py-1.5 lg:flex">
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-border/60 bg-card/40 text-muted-foreground backdrop-blur transition-all hover:border-primary/40 hover:bg-primary/10 hover:text-primary xl:hidden"
              >
